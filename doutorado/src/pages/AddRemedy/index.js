...
//Estados dos selects
    const [ frequencia, setFrequencia ] = useState({selected: ''});
    const [ receita, setReceita ] = useState({selected: ''});
    const [ duracao, setDuracao ] = useState({selected: ''});
    const [ instrucao, setInstrucao ] = useState({selected: ''});
    ...
    const { handleSubmit, setFieldValue, setSubmitting, isSubmitting, values, errors, touched } = useFormik({
        ...
        onSubmit: async (values) => {          
            try{
                ...
                await api.post('/medicamento/novo', values, { headers:{
                    authorization: userId
                }}).then((resp)=>{
                    if(resp.status === 200){
                        setSubmitting(false);
                        Alert.alert('Sucesso', 'Novo medicamento cadastrado')
                        setTimeout(()=>{
                            goHome()
                        }, 1500)
                    }
                ...
                Alert.alert('Erro', "Erro ao cadastrar novo medicamento.")
            }
        }
    })
...
return (
    <>
    <Header title="Adicionar Medicamento" setVisibleConfirm={setVisibleConfirm}/>
        <View style={style.container}>
            ...
            <ReturnModal visible={visibleConfirm} setVisibleConfirm={setVisibleConfirm} hideDialog={hideDialog}/>
                ...
                <TextInput  
                    ...
                    onFocus={()=> false ? setShowReceita(false) : setShowReceita(true)}
                />
